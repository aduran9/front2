...
function agregarReservation() {
    if($("#startDate").val().length == 0 || $("#devolutionDate").val().length == 0 || $("#status").val().length == 0){
        ...
        $.ajax({
            ...
            url:"http://144.22.242.145:8080/api/Reservation/save",
            data: dataToSend,
            datatype: "json",
...
function listarReservation(){
    ...
    $.ajax({
        url:"http://144.22.242.145:8080/api/Reservation/all",
        type: "GET",
        datatype: "JSON",
        success:function(response) {
            console.log(response);
    ...
    $.ajax(
        {
            dataType: 'json',
            data: dataToSend,
            url:"http://144.22.242.145:8080/api/Reservation/"+idElemento,
            type: 'DELETE',
            contentType: "application/JSON",
            success: function (response) {
                console.log(response);
...
function cargarDatosReservation(id) {
    ...
    $.ajax({
        dataType: 'json',
        url:"http://144.22.242.145:8080/api/Reservation/"+id,
        type: 'GET',
        success: function (response) {
            console.log(response);
            var item = response;
...
function actualizarReservation(idElemento) {
    if($("#startDate").val().length == 0 || $("#devolutionDate").val().length == 0 || $("#status").val().length == 0){
        ...
        $.ajax({
            ...
            data: dataToSend,
            contentType: "application/JSON",
            url:"http://144.22.242.145:8080/api/Reservation/update",
            type: "PUT",
            success: function (response) {
                console.log(response);
                $("#miListaReservation").empty();
